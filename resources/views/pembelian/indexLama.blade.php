...
<head>
    ...
    <!-- Menambahkan meta tag untuk responsivitas -->
    <style>
        .table-responsive {
            overflow-x: auto;
        }
                                            ...
                                            <td class="text-center">
                                                <div class="d-flex justify-content-center">
                                                    ...
                                                    @if (Auth::check() && Auth::user()->hasRole('admin'))
                                                        @if ($item->status == 1)
                                                            <a href="{{ url('pembelian/' . $item->id . '/edit-barang-baru') }}"
                                                                class="btn btn-primary btn-sm">
                                                                <i class="fas fa-edit"></i>
                                                                Edit
                                                            </a>
                                                        @else
                                                            <a href="{{ url('pembelian/' . $item->id . '/edit') }}"
                                                                class="btn btn-primary btn-sm">
                                                                <i class="fas fa-edit"></i>
                                                                Edit
                                                            </a>
                            ...
                            <tbody>
                                @foreach ($item->barangs as $barang)
                                    <tr class="text-center">
                                        ...
                                        <td class="col-md-2 text-center">Rp.
                                            {{ number_format($barang->pivot->harga, 0, ',', '.') }}</td>
                                        <td class="col-md-2 text-center">{{ $barang->pivot->jumlah }}</td>
                                        <td class="col-md-2 text-center">Rp.
                                            {{ number_format($barang->pivot->harga * $barang->pivot->jumlah, 0, ',', '.') }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
